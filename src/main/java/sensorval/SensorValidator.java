...
public class SensorValidator {
	public static boolean ifThreshold(double value, double nextValue, double maxDelta) {
	...
	public static boolean validateSOCreadings(List<Double> values) {
		return maxDeltaCheck(values, 0.5);
	}

	public static boolean validateCurrentreadings(List<Double> values) {
		return maxDeltaCheck(values, 0.1);
	}

	public static boolean maxDeltaCheck(List<Double> values, double maxDelta) {
	...
	public static boolean getMaxDeltaCheck(List<Double> values, int index, double maxDelta) {
		if (!nullCheck(values, index) && !ifThreshold(values.get(index), values.get(index + 1), maxDelta)) {
			return false;
		}
		return true;
	}
